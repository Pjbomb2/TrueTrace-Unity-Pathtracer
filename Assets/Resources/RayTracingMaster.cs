        ...
        if(uFirstFrame != 1) {
            if(DoTLASUpdates) {
                if(Assets.UpdateTLAS()) {
                    CreateComputeBuffer(ref _AggTriBuffer, Assets.aggregated_triangles, 136);
...
//I could try to optimize it by using a structured buffer that contains normals, colors, and positions, making it float3, float3, float2(compressed normal)
        if(!UseSVGF) {
