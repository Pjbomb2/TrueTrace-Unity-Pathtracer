...
inline float3 GetDisplacementNormal(Prism TP, float2 UV, MaterialData TempMat, const CudaTriangleA Tri, int2 TempUv) {
	...
	float3 Ns = cross(normalize(sc - sa), normalize(sb - sa));// / (PDMNormEpsilon * PDMNormEpsilon);
	float3 geometric_normal = (cross(normalize(Tri.posedge1), normalize(Tri.posedge2)));
    float wldScale = rsqrt(dot(geometric_normal, geometric_normal));
...
inline bool triangle_intersect_shadow(int tri_id, const SmallerRay ray, const float max_distance, inout float3 throughput, const int MatOffset, const int mesh_id) {
...
inline void triangle_intersect_dist(const int tri_id, const SmallerRay ray, inout float max_distance, const int MatOffset, const int mesh_id) {
    TrianglePos tri = triangle_get_positions(tri_id);
    #ifdef TTDisplacement
        const int MaterialIndex = (MatOffset + AggTrisA[tri_id].MatDat);
        if(_IntersectionMaterials[MaterialIndex].DisplacementFactor != 0 && _IntersectionMaterials[MaterialIndex].DisplacementTex.x != 0) {
        ...
        if(triangle_group.y != 0) {
            [branch]if (tlas_stack_size == -1) {//Transfer from Top Level Accelleration Structure to Bottom Level Accelleration Structure
				...
				while (triangle_group.y != 0) {                        
                    ...
                    triangle_intersect_dist(triangle_group.x + triangle_index, ray, dist, MatOffset, mesh_id);
#else
                    if(triangle_intersect_shadow(triangle_group.x + triangle_index, ray, dist, through, MatOffset, mesh_id)) return false;
#endif
                }
            }
        }
