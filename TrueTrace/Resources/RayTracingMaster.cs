    ...
    public class RayTracingMaster : MonoBehaviour
    {
        ...
        private void SetShaderParameters(CommandBuffer cmd)
        {
            ...
            if (true)
            {
                ...
                ReSTIRGI.SetTexture(GITemporalKernel, "RandomNums", _RandomNums);
                ReSTIRGI.SetTexture(GITemporalKernel, "TempNormTex", _NormTex);
                ReSTIRGI.SetTexture(GITemporalKernel, "PrevDepthTex", PrevDistanceTex);
        ...
        private void Render(RenderTexture destination, CommandBuffer cmd)
        {
            ...
            else if (!UseASVGF)
            {
                ...
                Denoisers.ExecuteSVGF(_currentSample, SVGFAtrousKernelSizes, ref _ColorBuffer, ref _target, ref _Albedo, ref _NormTex, RenderScale != 1.0f, ref PrevDistanceTex, cmd, ref CorrectedDistanceTex, UseReSTIRGI);
                CurrentSample = 1;
            }
            else
            {
                SampleCount = 0;
                ASVGFCode.Do(ref _ColorBuffer, ref _NormTex, ref _Albedo, ref _target, ref _RandomNums, RenderScale != 1.0f, ref PrevDistanceTex, ((FramesSinceStart2 % 2 == 0) ? MatModifierBuffer : MatModifierBufferPrev), cmd, ref CorrectedDistanceTex, ref RaysBuffer, FramesSinceStart2);
                CurrentSample = 1;
            }
            ...
            {
                if (UseTAAU) Denoisers.ExecuteTAAU(ref _FinalTex, ref _converged, ref _Albedo, cmd, FramesSinceStart2, ref CorrectedDistanceTex);
